...
export default function PiePopup({ selectedFood, selectedCountry }: FoodProps) {
  ...
  useEffect(() => {
    if(selectedFood?.name && selectedFood?.quantity) {
      ...
      const fetchData = async () => {
        ...
        console.log(`Total global food production of ${selectedFood?.name} is:`, totalFoodProduction)
  ...
  const drawPieChart = (data: number[], initial = false) => {
    ...
    const pie = d3.pie().value((d: any) => d).sort(null)
    ...
    text
      ...
      .text((d: any) => `${((d.data / globalFoodData) * 100).toFixed(2)}%`)
      .style("fill", "white")
  ...
  return isVisible ? (
    <div className="fixed inset-0 bg-gray-700 bg-opacity-50 flex justify-center items-center">
      <div className="bg-white rounded-lg shadow-lg p-12 relative flex justify-center items-center flex-col">
        <button onClick={handleClose} className="absolute top-2 right-2 text-blue-900 text-xl">✖</button>
        <p className="text-xl font-light text-green-600">Percentage of {selectedFood?.name}, {selectedCountry} in relation to global production</p>
